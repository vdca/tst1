#' ---
#' title: tsetting, NL, Liederenbank, s--p+
#' subtitle: filtered out -> function words, melisma, first/last in line
#' author: varun@ni.eus
...
dfilter <- d %>%
  filter(melismaTrans == '00', content %in% c(1),
         line.initial == 0, word.initial %in% c(0, 1),
...
dfilter %>% 
  select(songID, phraseID, syllable, word, clx.pos,
